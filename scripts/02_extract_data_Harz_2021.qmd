...
```{r}
rpast <- rast(im2list("D:\\wsf-sat\\methods\\reference_data\\Harz_2021\\Merge_UNC_UPC_2020_GTiff_shift_TC.tif"))
rpresent <- rast(im2list("D:\\wsf-sat\\methods\\reference_data\\Harz_2021\\Merge_UNC_UPC_2021_GTiff_shift_TC.tif"))
...
```{r}
# Geometries
damage_polygons <- vect(here("data", "Harz_Freiflaeche_stehendesTotholz_2021.gpkg"), layer="filter025")
...
damage_polygons_table <- damage_polygons_table %>% 
                            select("Jahr", "Typ_wsfsat")  %>% 
                            rename("damage_type" = "Typ_wsfsat") %>%
