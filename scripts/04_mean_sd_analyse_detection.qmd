...
```{r}
Index <- sub("\\_.*", "", dataplot$var) # remove _diff
ggplot(dataplot,aes(x=diff,y=sd,col=Index,label=Index))+geom_point()+
...
```{r}
ggplot(dataplot, aes(x = reorder(Index, -ratio), y = ratio, fill=Index)) + geom_col() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + xlab("Index") + ylab("Ratio")
```
